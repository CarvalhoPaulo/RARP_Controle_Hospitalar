...
public class CadastroEspecialidadeController extends Application implements Initializable {
	...
	public void altera(EspecialidadeCtrl ctrl) throws Exception {
		// TODO Auto-generated method stub

		especialidadeCtrl = ctrl;
		start(SistemaCtrl.getInstance().getStage());
		stage.setResizable(false);
		stage.showAndWait();
	}
	...
	private void PreencherObjeto() {
		...
		especialidadeCtrl.getEspecialida().setNome(txtNome.getText());
		especialidadeCtrl.getEspecialida().setObservacoes(txtObservacoes.getText());
		especialidadeCtrl.getEspecialida().setCodigo(txtCodigo.getValue());
		especialidadeCtrl.getEspecialida().setNome(txtNome.getText());
	}
